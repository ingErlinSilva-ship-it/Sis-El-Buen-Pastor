<aside
    ...
    <div class="user-panel-bottom py-3 border-top border-secondary">
        <div class="container-fluid px-3">
            ...
            {{-- Card de Usuario Azul con Enlace Directo --}}
            <a href="{{ route('usuario.show', Auth::id()) }}" class="user-info-link" style="text-decoration: none;">
                <div class="user-info-card shadow-sm mb-3"
                    style="background-color: #0b4c81; border-radius: 12px; overflow: hidden; transition: transform 0.2s;">
                    <div class="d-flex align-items-center p-2">
                        ...
                        <div class="info text-truncate">
                            <small class="text-uppercase d-block font-weight-bold"
                                style="font-size: 0.6rem; color: #94a3b8; line-height: 1;">Mi Perfil</small>

                            <span class="font-weight-bold text-white d-block text-truncate" style="font-size: 0.85rem;"
                                title="{{ Auth::user()->nombre }} {{ Auth::user()->apellido }}">
                                {{ explode(' ', trim(Auth::user()->nombre))[0] }}
                            </span>

                            <span class="badge badge-info p-0 px-1"
                                style="font-size: 0.65rem; background: rgba(56, 189, 248, 0.2); color: #38bdf8; text-transform: capitalize;">
            ...
            <form action="{{ route('logout') }}" method="POST" class="m-0">
                @csrf
                <button type="submit"
                    class="btn btn-danger btn-sm btn-block d-flex align-items-center justify-content-center py-2 shadow-sm"
    ...
    <style>
        /* Efecto al pasar el mouse por la card del perfil */
        .user-info-card:hover {
            background-color: #0d5ca0 !important;
            /* Un azul un poco más claro */
            transform: translateY(-2px);
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2) !important;
        }

        /* Aseguramos que el enlace no tenga subrayado */
